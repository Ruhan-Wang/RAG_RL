...
def evaluate_logprob_with_retrieved_docs(
    ...
    # 评估模型，不更改梯度
    with torch.no_grad():
        # 放入model得到预测结果的logits
        lm_logits = model(input_ids).logits

        # 通过logits按照不同的策略选取doc
...
def eval_dataset(
    ...
    dataset_len = encodings.input_ids.size(1) # 获取输入序列长度
    print("Dataset length:", dataset_len)

    # Nomalization factor, 具体的选取影响ppl的计算，默认word
